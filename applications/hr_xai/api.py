"""HR XAI — FastAPI router.

ML + explainability for HR Attrition. Mirrors loan_xai structure.
Uses hr_xai data_loader + hr_ml services + loan_xai explainer (reused).
...
from applications.hr_ml.services import exploration, metrics, preprocessor
...
from applications.hr_ml.services.trainer import (
    SUPPORTED_MODELS, TrainingError, train, get_hyperparameter_defaults,
)
from applications.loan_xai.services.explainer import (
    ExplainerError, build_explanation, explain_instance_lime,
)

router = APIRouter(prefix="/api/hr-xai", tags=["HR XAI"])
...
async def upload_dataset(file: UploadFile = File(...)):
    ...
    try:
        dataset = load_csv(file.filename or "upload.csv", content)
...
def explore_dataset(session_id: str):
    ...
    result = exploration.explore(df)
    return {
        "rows": result.shape[0], "columns": result.shape[1],
        "missing_values": int(result.missing.sum()),
...
def explain(session_id: str, req: ExplainRequest):
    """SHAP global importance + LIME local explanation for an HR Attrition test instance."""
    session = _get_session(session_id)
    ...
    try:
        explain_result = build_explanation(tr.model, pr.X_train, pr.X_test, class_names, tr.model_name)
    ...
    return {
        ...
        "lime_explanation": [{"feature": f, "weight": round(float(w), 6)} for f, w in lime_pairs],
...
def download_bundle(session_id: str):
    ...
    return StreamingResponse(buf, media_type="application/octet-stream",
                             headers={"Content-Disposition": "attachment; filename=hr_xai_bundle.pkl"})
...
def _get_session(session_id: str) -> Session:
    try:
        ...
        raise HTTPException(status_code=404,
                            detail=f"Session '{session_id}' not found. Call POST /api/hr-xai/upload first.")
