"""UC7 — Upload Docs page. Same pattern as UC1–UC6."""
...
def render() -> None:
    ...
    st.write(
        ...
        "After loading, go to **Configure** to choose which retrieval modules to activate."
    )
    ...
    if docs:
        ...
        if vs:
            ...
            st.success("Vector store ready. Go to **Configure** to choose modules.")
    else:
...
def _build_and_store(loaded_docs: list[LoadedDoc]) -> None:
    ...
    with st.spinner("Embedding into ChromaDB…"):
        try:
            vs = build_vector_store(chunks, doc_count=len(loaded_docs), collection_name="rag_uc7")
        except Exception as exc:
