"""UC6 — Upload Docs page. Same pattern as UC1–UC5."""
...
def render() -> None:
    ...
    st.write(
        ...
        "CRAG will retrieve chunks from these documents, grade each one, "
        "and fall back to Wikipedia if the documents don't cover your question."
    )
...
def _build_and_store(loaded_docs: list[LoadedDoc]) -> None:
    ...
    with st.spinner("Embedding into ChromaDB…"):
        try:
            vs = build_vector_store(chunks, doc_count=len(loaded_docs), collection_name="rag_uc6")
        except Exception as exc:
