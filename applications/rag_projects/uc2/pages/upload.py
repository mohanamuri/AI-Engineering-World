"""
UC2 — Upload Docs page.

Same document loading as UC1 with one important addition:
after building the ChromaDB vector store, a BM25Retriever is built from
the same chunks and stored in session state. The Chat page needs both
...
from applications.rag_projects.services.hybrid_retriever import BM25Retriever
...
def render() -> None:
    ...
    st.write(
        ...
        "Both a ChromaDB vector store (dense) **and** a BM25 keyword index are built from the same "
        "chunks — enabling hybrid retrieval with RRF fusion in Chat."
    )
    ...
    with col_sample:
        with st.container(border=True):
            ...
            st.caption("3 ACME corporate policy documents — great for hybrid retrieval demos")
    ...
    if docs:
        ...
        if vs:
            ...
            c4.metric("BM25 index", "✅ Ready" if bm25 else "❌ Missing")
            st.success("Both retrievers ready. Go to **Configure** or **Chat**.")
    else:
        st.info("Load sample documents or upload your own to get started.")


def _load_sample_docs() -> None:
    """Load all 3 bundled sample documents and build both indexes."""
...
def _load_uploaded_files(uploaded_files) -> None:
    """Load user-uploaded files and build both indexes."""
    loaded_docs: list[LoadedDoc] = []
...
def _build_and_store(loaded_docs: list[LoadedDoc]) -> None:
    """Chunk all docs, build vector store + BM25 index, save to session state."""
    ...
    with st.spinner("Building BM25 keyword index…"):
        try:
            ...
            st.error(f"Failed to build BM25 index: {exc}")
            return
    ...
    st.session_state[BM25_RETRIEVER_SESSION_KEY] = bm25
    st.session_state.pop(CHAT_HISTORY_SESSION_KEY, None)
    st.session_state.pop(RAG_CONFIG_SESSION_KEY, None)

    st.success(
        f"Loaded {len(loaded_docs)} document(s) → {len(chunks)} chunks "
        "embedded into ChromaDB + BM25 index built."
    )
