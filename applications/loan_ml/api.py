"""
Loan ML — FastAPI router.

Endpoints walk through the same pipeline as the Streamlit app:
  POST /api/loan-ml/upload            → upload CSV, get session_id
  GET  /api/loan-ml/{sid}/explore     → dataset profile
  POST /api/loan-ml/{sid}/preprocess  → fit preprocessing pipeline
  POST /api/loan-ml/{sid}/train       → train a model
  GET  /api/loan-ml/{sid}/evaluate    → full metrics
  POST /api/loan-ml/{sid}/predict     → predict for new input
  GET  /api/loan-ml/{sid}/download/model   → download .pkl
  GET  /api/loan-ml/{sid}/download/bundle  → download model + pipeline bundle

All ML work is done by the existing services/ layer — nothing here is rewritten.
"""
...
from applications.loan_ml.services import data_loader, exploration, metrics, preprocessor, trainer
...
from applications.loan_ml.services.trainer import SUPPORTED_MODELS, TrainingError

router = APIRouter(prefix="/api/loan-ml", tags=["Loan ML"])
...
def list_models():
    """List supported model types and their default hyperparameters."""
    return {
        "supported_models": SUPPORTED_MODELS,
        "hyperparameter_defaults": {
            m: trainer.get_hyperparameter_defaults(m) for m in SUPPORTED_MODELS
        },
    }
...
async def upload_dataset(file: UploadFile = File(...)):
    """
    Upload a CSV dataset. Returns a **session_id** you must pass to all
    subsequent endpoints for this pipeline run.
    """
    ...
    return {
        ...
        "columns": dataset.dataframe.shape[1],
...
def explore_dataset(session_id: str):
    ...
    Returns shape, missing values, column types, and the auto-detected target column.
    """
...
def preprocess_dataset(session_id: str, req: PreprocessRequest):
    ...
    - Fits the pipeline on train split only (no data leakage)
    - Stores the fitted pipeline in the session for prediction
    """
    session = _get_session(session_id)
    df = _require_dataframe(session)
    ...
    # Track original column order for the /predict endpoint
    ...
    return {
        "train_rows": result.X_train.shape[0],
        "test_rows": result.X_test.shape[0],
...
def train_model(session_id: str, req: TrainRequest):
    ...
    Supported model names: Logistic Regression, Decision Tree, Random Forest, XGBoost.
    If `hyperparams` is omitted, sensible defaults are used (see /models).
    """
    session = _get_session(session_id)
    if session.preprocess_result is None:
        raise HTTPException(status_code=400, detail="Run /{session_id}/preprocess first.")
    ...
    eval_result = metrics.evaluate(result.model, pr.X_test, pr.y_test)
...
def evaluate_model(session_id: str):
    """
    Full evaluation metrics for the trained model:
    accuracy, precision, recall, F1, ROC-AUC, confusion matrix, per-class report.
    """
    session = _get_session(session_id)
    if session.eval_result is None:
        raise HTTPException(status_code=400, detail="Run /{session_id}/train first.")
...
def predict(session_id: str, req: PredictRequest):
    ...
    Provide **raw, unscaled** feature values exactly as they appear in the
    original CSV (the preprocessing pipeline handles scaling/encoding).
    ...
    if session.train_result is None or session.preprocess_result is None:
        raise HTTPException(
            status_code=400,
            detail="Run /{session_id}/preprocess and /{session_id}/train first.",
        )
...
def download_model(session_id: str):
    """Download the trained model as a `.pkl` file (joblib serialized)."""
    session = _get_session(session_id)
...
def download_bundle(session_id: str):
    """
    Download a bundle `.pkl` containing the model + fitted preprocessing pipeline.
    Load it with `joblib.load()` — use `bundle['pipeline']` to transform new data
    before passing to `bundle['model'].predict()`.
    """
    session = _get_session(session_id)
    if session.train_result is None or session.preprocess_result is None:
        raise HTTPException(
    ...
    return StreamingResponse(
        buf,
        media_type="application/octet-stream",
        headers={"Content-Disposition": "attachment; filename=loan_ml_bundle.pkl"},
    )
...
def _get_session(session_id: str) -> Session:
    try:
        ...
        raise HTTPException(
            status_code=404,
            detail=f"Session '{session_id}' not found. Call POST /api/loan-ml/upload first.",
        )
