"""Dataset exploration dashboard for the loan eligibility application."""
...
def render() -> None:
    """Render the exploration dashboard for the session dataset."""
...
def _render_empty_state() -> None:
    with st.container(border=True):
        ...
        st.write(
            "Upload and validate a CSV dataset before opening the exploration "
            "dashboard."
        )
        st.button(
...
def _render_dataset_summary(profile: DatasetProfile) -> None:
    ...
    for column, (label, value, help_text) in zip(
        metric_columns, metrics, strict=True
    ):
        with column:
            with st.container(border=True):
...
def _render_column_list(columns: tuple[str, ...]) -> None:
    if columns:
        st.write(" · ".join(f"`{column}`" for column in columns))
    else:
...
def _render_statistical_summary(dataframe: pd.DataFrame) -> None:
    ...
    include_categorical = st.checkbox(
        ...
        value=False,
        key="loan_ml_include_categorical_summary",
    )
...
def _render_missing_values(
    ...
    with total_column:
        with st.container(border=True):
            ...
            missing_rate = (
                profile.missing_values / populated_cells * 100
                if populated_cells
                else 0
            )
    ...
    with detail_column:
        styled_summary = missing_summary.style.apply(
            _highlight_missing_row,
            axis=1,
        ).format({"Missing (%)": "{:.2f}%"})
        st.dataframe(
            styled_summary,
...
def _render_target_distribution(dataframe: pd.DataFrame) -> None:
    ...
    if target_column is None:
        st.info(
            "No recognized target column was detected. Expected a column such "
            "as `LoanApproved` or `Loan_Status`."
        )
        return
...
def _render_numeric_distribution(
    dataframe: pd.DataFrame,
    profile: DatasetProfile,
) -> None:
...
def _render_preview_and_download(dataframe: pd.DataFrame) -> None:
    ...
    with st.container(border=True):
        ...
        st.caption(
            f"Showing {min(preview_rows, len(dataframe)):,} of "
