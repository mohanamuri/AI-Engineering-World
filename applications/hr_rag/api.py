"""HR RAG — FastAPI router.

Document Q&A pipeline: upload → configure → chat → history
Uses ChromaDB + HuggingFace embeddings + Groq LLM (HR policy assistant).
...
from applications.hr_rag.services.vector_store import build_vector_store
from applications.hr_rag.services.rag_chain import RAGConfig, run_rag_query

router = APIRouter(prefix="/api/hr-rag", tags=["HR RAG"])
...
async def upload_document(file: UploadFile = File(...)):
    """Upload a PDF or TXT HR policy document. Returns session_id for subsequent calls."""
...
def configure(session_id: str, req: ConfigureRequest):
    ...
    if session.load_result is None:
        raise HTTPException(status_code=400, detail="Upload a document first via POST /api/hr-rag/upload.")
...
def chat(session_id: str, req: ChatRequest):
    """Ask a question against the indexed HR policy document. Requires /configure first."""
    session = _get_session(session_id)
...
def _get_session(session_id: str) -> Session:
    try:
        ...
        raise HTTPException(
            status_code=404,
            detail=f"Session '{session_id}' not found. Call POST /api/hr-rag/upload first.",
        )
