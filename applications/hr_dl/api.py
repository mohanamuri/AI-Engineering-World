"""HR DL — FastAPI router.

Neural network pipeline for HR Attrition. Mirrors loan_dl structure.
Uses hr_dl data_loader + hr_ml preprocessor/metrics + hr_dl trainer.
...
from applications.hr_dl.services.trainer import (
    ARCHITECTURES, DLTrainResult, DLTrainingError, train,
)

router = APIRouter(prefix="/api/hr-dl", tags=["HR DL"])
...
def download_bundle(session_id: str):
    ...
    return StreamingResponse(buf, media_type="application/octet-stream",
                             headers={"Content-Disposition": "attachment; filename=hr_dl_bundle.pkl"})
...
def _get_session(session_id: str) -> Session:
    try:
        ...
        raise HTTPException(status_code=404,
                            detail=f"Session '{session_id}' not found. Call POST /api/hr-dl/upload first.")
