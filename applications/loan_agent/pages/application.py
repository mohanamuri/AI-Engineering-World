"""Loan application form — page 1 of the loan agent workflow."""
...
_SAMPLES = {
    ...
    "Borderline applicant (likely MANUAL_REVIEW)": {
    ...
    "Weak applicant (likely DECLINED)": {
...
def render() -> None:
    ...
    with st.form("loan_application_form"):
        ...
        with col2:
            income = st.number_input(
                "Monthly income (USD)", min_value=0, max_value=500_000,
                value=int(saved.get("monthly_income_usd", 5000)), step=100,
            )
            existing_debt = st.number_input(
                "Existing monthly debt payments (USD)", min_value=0, max_value=50_000,
        ...
        with col3:
            ...
            loan_amount = st.number_input(
                "Loan amount (USD)", min_value=1000, max_value=500_000,
                value=int(saved.get("loan_amount_usd", 10000)), step=500,
            )
        with col4:
            tenure = st.number_input(
                "Loan tenure (months)", min_value=12, max_value=180,
    ...
    if submitted:
        application = {
            ...
            "existing_monthly_debt_usd": existing_debt,
            "credit_score": credit_score,
            "loan_type": loan_type,
            "loan_amount_usd": loan_amount,
            "loan_tenure_months": tenure,
        }
        st.session_state[APPLICATION_SESSION_KEY] = application
        st.success("Application saved. Go to **Run Agent** to evaluate it.")
